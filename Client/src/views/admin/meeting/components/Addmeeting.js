...
const AddMeeting = (props) => {
    const { onClose, isOpen, setAction, from, fetchData, id, view, data } = props
    ...
    const initialValues = {
        agenda: '',
        attendes: props.leadContect === 'contactView' && id ? [id] : [],
        attendesLead: props.leadContect === 'leadView' && id ? [id] : [],
        location: '',
    ...
    /*
        Logic to add a new meeting from Client via restful
    */
    const AddData = async () => {
        try {
            setIsLoding(true);
            ...
            console.log("error adding meeeting: ", e);
        } finally {
            setIsLoding(false);
        }
    };
    ...
    // Meetings home page is not immediately populating with new meeting after creation
    // - need to check if this is directly from pre-written code or something i need to do
    const fetchAllData = async () => {
        if (id) {
            try {
                setIsLoding(true);
                let result = await getApi('api/meeting/', id);
                ...
                console.log("error fetching meeting data: ", e);
            } finally {
                setIsLoding(false);
            }
        } else if (data) {
